...
| `GET /api/servers/{id}/volumes?slot_id=…` | Server-local storage keyed by placement + user-given deploy name (`project_name`) + mount name, with creator, management rights and attachments. `slot_id` returns that slot plus SERVER roots; `gpu_group_id` addresses a group slot; omitting both lists every placement. `project_name` is not a GitLab project | ✅ live (placement scope 0.63.0) |
