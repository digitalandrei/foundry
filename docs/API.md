...
| `GET /api/registry/tags/{tag_id}/metadata` | Deploy defaults read from the image config (Registry v2: manifest → config; linux/amd64 picked from multi-arch indexes): EXPOSE ports, compressed layer size, and persistent mounts from standard Docker `VOLUME` plus the richer `ai.protv.foundry.volumes` JSON label. Best-effort: failures return empty editable defaults. The old `/exposed-ports` path remains a response-compatible alias | ✅ live (0.53.0) |
